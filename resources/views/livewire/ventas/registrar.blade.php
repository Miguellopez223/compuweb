            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <div class="relative mb-3">
                        <label class="block text-xs font-medium text-slate-600 mb-1">Buscar cliente existente (nombre o CI/NIT)</label>
                        <div class="flex gap-2">
                            <div class="relative flex-1">
                                <input wire:model.live.debounce.300ms="busquedaCliente" type="text"
                                       placeholder="Escribe para buscar..."
                                       wire:keydown.escape="cerrarResultados"
                                       class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 pr-8">
                            ...
                            @if($cliente_id)
                                <button wire:click="limpiarCliente" type="button"
                                        class="px-3 py-2 text-xs font-medium text-red-600 border border-red-300 rounded-lg hover:bg-red-50 transition">
                                    Limpiar
                                </button>
                            @endif
                        </div>
                        ...
                        <div class="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            @foreach($resultadosClientes as $resultado)
                            <button wire:click="seleccionarCliente({{ $resultado['id'] }})" type="button"
                                    class="w-full px-3 py-2.5 text-left hover:bg-indigo-50 transition border-b border-slate-100 last:border-b-0">
                                <div class="flex items-center justify-between">
                                    <span class="text-sm font-medium text-slate-800">{{ $resultado['nombre'] }}</span>
                                    @if($resultado['ci_nit'])
                                        <span class="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded">{{ $resultado['ci_nit'] }}</span>
                                    @endif
                                </div>
                        ...
                        <div class="absolute z-20 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg p-3">
                            <p class="text-xs text-slate-500 text-center">No se encontraron coincidencias. Se creara un cliente nuevo.</p>
                        </div>
                        @endif
                    </div>

                    @if($cliente_id)
                    <div class="bg-indigo-50 border border-indigo-200 rounded-lg px-3 py-2 mb-3 flex items-center gap-2">
                        ...
                        <span class="text-xs text-indigo-700 font-medium">Cliente seleccionado: {{ $cliente_nombre }}</span>
                    </div>
                    @endif
