<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-slate-500">Gestiona las categorías de productos y asigna especialistas</p>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 border-b border-slate-200">
                    <tr>
                        ...
                        <th class="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Especialista WhatsApp</th>
                ...
                <tbody class="divide-y divide-slate-100">
                            ...
                            @if($cat->especialista)
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center">
                                        <span class="text-xs font-bold text-indigo-600">{{ strtoupper(substr($cat->especialista->name, 0, 1)) }}</span>
                                    </div>
                                    <span class="text-slate-700">{{ $cat->especialista->name }}</span>
                                </div>
                            @else
                    ...
                    <tr>
                        <td colspan="4" class="px-5 py-12 text-center text-slate-400">
                            ...
                            <p class="text-sm mt-1">Crea la primera categoría para organizar tus productos</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div class="px-6 py-5 space-y-4">
                <div>
                    ...
                    <input wire:model="nombre" type="text" placeholder="Ej: CPU, GPU, RAM..."
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Especialista WhatsApp</label>
                    <select wire:model="especialista_id"
                        ...
                        @foreach($especialistas as $e)
                            <option value="{{ $e->id }}">{{ $e->name }} @if($e->whatsapp_number)({{ $e->whatsapp_number }})@endif</option>
                        @endforeach
                    </select>
                    <p class="mt-1 text-xs text-slate-400">El especialista recibirá las consultas de WhatsApp para esta categoría</p>
                </div>
            </div>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h3 class="text-base font-semibold text-slate-800 mb-2">¿Eliminar categoría?</h3>
            <p class="text-sm text-slate-500 mb-6">Esta acción eliminará también los productos asociados. No se puede deshacer.</p>
