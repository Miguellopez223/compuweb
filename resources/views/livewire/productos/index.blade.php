            ...
            <div class="px-6 py-5 space-y-4 overflow-y-auto flex-1">
                ...
                <div>
                    ...
                    @if($imagen)
                        ...
                        <p class="mt-1.5 text-xs text-slate-400">{{ $imagen->getClientOriginalName() }} · {{ round($imagen->getSize() / 1024) }} KB</p>
                    @elseif($imagenActual)
                        {{-- Imagen existente en BD --}}
                        <div class="relative inline-block">
                            <img src="{{ Storage::url($imagenActual) }}" alt="Imagen actual"
                                 class="w-full h-40 object-cover rounded-xl border border-slate-200">
                            <span class="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-0.5 rounded-full">Imagen actual</span>
                        </div>
                        <div class="mt-2">
                            <label for="img-upload"
                                   class="inline-flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800 cursor-pointer font-medium">
                    ...
                    @else
                        {{-- Zona de carga vacía --}}
                        <label for="img-upload"
                               class="flex flex-col items-center justify-center w-full h-36 border-2 border-dashed border-slate-300 rounded-xl cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition group">
                            ...
                            <p class="text-sm text-slate-500 group-hover:text-indigo-600 font-medium transition">Haz clic para subir imagen</p>
                            <p class="text-xs text-slate-400 mt-0.5">PNG, JPG, WEBP · Máx 2 MB</p>
                        </label>
                        <input id="img-upload" wire:model="imagen" type="file" accept="image/*" class="hidden">
                    @endif
